...
*/
static const char *lockStateName(int lock){
  switch(lock){
    case 0: return "NO_LOCK";
...
static int unixLock(sqlite3_file *id, int eFileLock){
  ...
  */
  if( eFileLock==SHARED_LOCK &&
    ...
    fprintf(stderr, "[%ld.%09ld] LOCK pid=%d fd=%d: %s -> SHARED\n", ts.tv_sec, ts.tv_nsec, osGetpid(0), pFile->h, lockStateName(pFile->eFileLock));
  ...
  if( eFileLock==SHARED_LOCK
   || (eFileLock==EXCLUSIVE_LOCK && pFile->eFileLock==RESERVED_LOCK)
    ...
    if( unixFileLock(pFile, &lock) ){
      ...
      struct timespec ts;
      clock_gettime(CLOCK_REALTIME, &ts);
      fprintf(stderr, "[%ld.%09ld] LOCK pid=%d fd=%d: %s -> PENDING\n", ts.tv_sec, ts.tv_nsec, osGetpid(0), pFile->h, lockStateName(pFile->eFileLock));
  ...
  */
  if( eFileLock==SHARED_LOCK ){
    ...
    if( rc ){
      ...
      struct timespec ts;
      clock_gettime(CLOCK_REALTIME, &ts);
      fprintf(stderr, "[%ld.%09ld] LOCK pid=%d fd=%d: %s -> SHARED (fallback)\n", ts.tv_sec, ts.tv_nsec, osGetpid(0), pFile->h, lockStateName(pFile->eFileLock));
      pFile->eFileLock = SHARED_LOCK;
      pInode->nLock++;
      pInode->nShared = 1;
    }
  ...
  */
  ...
  if( rc==SQLITE_OK ){
    struct timespec ts;
    clock_gettime(CLOCK_REALTIME, &ts);
    fprintf(stderr, "[%ld.%09ld] LOCK pid=%d fd=%d: %s -> %s\n", ts.tv_sec, ts.tv_nsec, osGetpid(0), pFile->h, lockStateName(pFile->eFileLock), lockStateName(eFileLock));
...
end_unlock:
  ...
  if( rc==SQLITE_OK ){
    struct timespec ts;
    clock_gettime(CLOCK_REALTIME, &ts);
    fprintf(stderr, "[%ld.%09ld] UNLOCK pid=%d fd=%d: %s -> %s\n", ts.tv_sec, ts.tv_nsec, osGetpid(0), pFile->h, lockStateName(pFile->eFileLock), lockStateName(eFileLock));
    pFile->eFileLock = eFileLock;
  }
  return rc;
}
...
static int dotlockUnlock(sqlite3_file *id, int eFileLock) {
  ...
  */
  ...
  struct timespec ts;
  clock_gettime(CLOCK_REALTIME, &ts);
  fprintf(stderr, "[%ld.%09ld] UNLOCK pid=%d fd=%d: %s -> NO_LOCK\n", ts.tv_sec, ts.tv_nsec, osGetpid(0), pFile->h, lockStateName(pFile->eFileLock));
  pFile->eFileLock = NO_LOCK;
  return SQLITE_OK;
}
